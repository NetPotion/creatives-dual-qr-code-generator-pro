<?php
/**
 * Bundled third-party library: QRCode for PHP
 *
 ...
 * 1. Wrapped in the CreativesDQRCGP namespace, with global define()
 *    constants converted to namespaced const declarations, so its classes and
 *    constants (QRCode, QRUtil, QR_PAD0, etc.) cannot collide with the same
 *    library if another active plugin bundles it under the global namespace.
 * 2. The character-capacity table (QRUtil::$QR_MAX_LENGTH) extended from
 *    versions 1-10 to all 40 versions, derived from the library's own
 *    Reed-Solomon block table (plugin 2.1.0).
 * 3. The unused printHTML() and printSVG() output methods removed; the plugin
 *    renders its own escaped output from the module matrix.
 *
 * Runs entirely locally — no network calls, no external API.
 *
 * @package Creatives_DQRCGP_Generator
 */

namespace CreativesDQRCGP;
...
class QRUtil {
    ...
    /*
     * Character-capacity table, versions 1-40.
     *
     * The upstream bundle shipped only versions 1-10, which silently
     * capped every QR code this plugin could produce and made
     * getMinimumQRCode() run off the end of the array (fatal) for any
     * payload needing version 11 or higher. Rows 11-40 are derived from
     * QRRSBlock::$QR_RS_BLOCK_TABLE below - the same block layout this
     * encoder actually uses - so capacity and encoding cannot drift
     * apart. The derivation reproduces the original rows 1-10 exactly.
     *
     * Columns: L, M, Q, H. Within each: numeric, alphanumeric, 8-bit, kanji.
     */
